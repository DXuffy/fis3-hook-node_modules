...
function followPath(path) {
  ...
  if(paths.length === 3){
    paths = [ paths[0], [ paths[1], paths[2] ].join('/') ];
  }
  ...
  function find(root, next) {
    ...
    var lastIndex = module.lastIndexOf('@');
    var moduleMap = [module.substr(0, lastIndex), module.substr(lastIndex + 1)];

    var name = moduleMap[0];
    var version = moduleMap[1];
    var findOne;
...
function onFileLookUp(info, file) {
  ...
  // if(rest.indexOf('@spider') !== -1){
    // m = /^(@spider\/[0-9a-zA-Z\.\-_]+(\/[0-9a-zA-Z\.\-_]+)?)$/.exec(rest);
  // }else{
    m = /^([@0-9a-zA-Z\.\-_]+)(?:\/(.+))?$/.exec(rest);
  // }
  ...
  if (m) {
    ...
    if (moduleVersion[cName]) {
      ...
      var maps = moduleMaps[cName].filter(function (val) {
        return val.indexOf('@') === -1 && val.replace(rleaModulePath, '');
      ...
      // try{
        // filePath = resolve.sync(rest + '.css', {basedir: moduleRoot})
      // }catch(e){
